...
if [[ -s "$3"/"$1"--"$5".fasta ]]; then
	...
	# Get best primer match
	fwd=$(cut -f10 "$3"/"$1"_"$5".fwd.sam.rc)
	rev=$(cut -f10 "$3"/"$1"_"$5".rev.sam.rc)
	myprimer=$(echo -e $fwd"\t"$rev)
	bestprimer=$(grep "$myprimer" "$6" | cut -f2)

	newheader=$(echo "$1"--"$bestprimer")

	#filename=$(basename "$3"/"$1"--"$5".fasta .fasta)
	awk -v name="$newheader" '/^>/ {print ">"name} !/^>/ {print}' "$3"/"$1"--"$5".fasta > "$3"/"$1"--"$5".fasta.2
	mv "$3"/"$1"--"$5".fasta.2 "$3"/"$1"--"$5".fasta

	# Change primer name to bestprimer when extracting from sam
	awk -F "\t" '{ print $3"--""'$bestprimer'", "'$bestprimer'", $4, $10 }' OFS="\t" "$3"/"$1"_"$5".fwd.sam.rc | sort -k1,1 > "$3"/"$1"_"$5".fwd
	awk -F "\t" '{ print $3"--""'$bestprimer'", "'$bestprimer'", $4+length($10)-1, $10 }' OFS="\t" "$3"/"$1"_"$5".rev.sam.rc | sort -k1,1 > "$3"/"$1"_"$5".rev
